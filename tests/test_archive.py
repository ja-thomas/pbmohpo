...
def test_archive_after_run():
    ...
    assert torch_values[1].shape == (10, 1)  # FIXME: not consistent with np_values[1] shape
    assert type(benchmark.archive.max_utility) == np.float64 # FIXME: could also check other types e.g. objectives, np_values, torch_values
